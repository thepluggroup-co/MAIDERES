...
-- Interdire UPDATE et DELETE via trigger
...
INSERT INTO rbac_roles (name, label, description, is_system) VALUES
  ...
  ('MANAGER',     'Manager',              'Gestion complète hors ADMIN/CONFIGURE',          true),
  ('COMMERCIAL',  'Commercial',           'Ventes, devis, commandes, stock lecture',        true),
  ('CAISSIER',    'Caissier',             'Encaissements et créances uniquement',           true),
  ('MAGASINIER',  'Magasinier',           'Gestion stocks et logistique',                  true),
  ('FORMATEUR',   'Formateur',            'Ressources humaines et rapports lecture',        true),
  ('READONLY',    'Lecture seule',        'Consultation uniquement, aucune écriture',      true)
ON CONFLICT (name) DO NOTHING;

-- ── Seed : Permissions (module × action) ─────────────────────────────────────

INSERT INTO rbac_permissions (module, action, label, is_immutable) VALUES
  ('STOCK',       'READ',      'Voir les stocks',                false),
  ('STOCK',       'CREATE',    'Créer des mouvements stock',     false),
  ('STOCK',       'UPDATE',    'Modifier les stocks',            false),
  ('STOCK',       'DELETE',    'Supprimer des articles stock',   false),
  ('STOCK',       'VALIDATE',  'Valider des entrées/sorties',    false),
  ('STOCK',       'CONFIGURE', 'STOCK — CONFIGURE',              false),
  ('STOCK',       'EXPORT',    'Exporter l''inventaire',         false),
  ('COMMERCIAL',  'READ',      'Voir devis et commandes',        false),
  ('COMMERCIAL',  'CREATE',    'Créer devis/commandes',          false),
  ('COMMERCIAL',  'UPDATE',    'Modifier devis/commandes',       false),
  ('COMMERCIAL',  'DELETE',    'Supprimer devis/commandes',      false),
  ('COMMERCIAL',  'VALIDATE',  'Valider devis/commandes',        false),
  ('COMMERCIAL',  'CONFIGURE', 'COMMERCIAL — CONFIGURE',         false),
  ('COMMERCIAL',  'EXPORT',    'Exporter commandes',             false),
  ('FINANCE',     'READ',      'Voir les finances',              false),
  ('FINANCE',     'CREATE',    'Créer factures/crédits',         false),
  ('FINANCE',     'UPDATE',    'Modifier factures',              false),
  ('FINANCE',     'DELETE',    'Supprimer écritures',            false),
  ('FINANCE',     'VALIDATE',  'Valider paiements',              false),
  ('FINANCE',     'CONFIGURE', 'FINANCE — CONFIGURE',            false),
  ('FINANCE',     'EXPORT',    'Exporter comptabilité',          false),
  ('HR',          'READ',      'Voir les employés',              false),
  ('HR',          'CREATE',    'Créer dossiers RH',              false),
  ('HR',          'UPDATE',    'Modifier RH',                    false),
  ('HR',          'DELETE',    'Supprimer RH',                   false),
  ('HR',          'VALIDATE',  'Valider bulletins',              false),
  ('HR',          'CONFIGURE', 'HR — CONFIGURE',                 false),
  ('HR',          'EXPORT',    'Exporter RH',                    false),
  ('PRODUCTION',  'READ',      'Voir production',                false),
  ('PRODUCTION',  'CREATE',    'Créer jobs production',          false),
  ('PRODUCTION',  'UPDATE',    'Modifier production',            false),
  ('PRODUCTION',  'DELETE',    'Supprimer jobs',                 false),
  ('PRODUCTION',  'VALIDATE',  'Valider jobs',                   false),
  ('PRODUCTION',  'CONFIGURE', 'PRODUCTION — CONFIGURE',         false),
  ('PRODUCTION',  'EXPORT',    'Exporter production',            false),
  ('LOGISTICS',   'READ',      'Voir livraisons',                false),
  ('LOGISTICS',   'CREATE',    'Créer livraisons',               false),
  ('LOGISTICS',   'UPDATE',    'Modifier livraisons',            false),
  ('LOGISTICS',   'DELETE',    'Supprimer livraisons',           false),
  ('LOGISTICS',   'VALIDATE',  'Valider livraisons',             false),
  ('LOGISTICS',   'CONFIGURE', 'LOGISTICS — CONFIGURE',          false),
  ('LOGISTICS',   'EXPORT',    'Exporter logistique',            false),
  ('ADMIN',       'READ',      'Lire la config admin',           false),
  ('ADMIN',       'CREATE',    'Créer utilisateurs',             false),
  ('ADMIN',       'UPDATE',    'Modifier utilisateurs',          false),
  ('ADMIN',       'DELETE',    'Supprimer utilisateurs',         true),
  ('ADMIN',       'VALIDATE',  'Valider actions admin',          false),
  ('ADMIN',       'CONFIGURE', 'Configurer le système',          true),
  ('ADMIN',       'EXPORT',    'Exporter logs audit',            false),
  ('REPORTS',     'READ',      'Voir les rapports',              false),
  ('REPORTS',     'CREATE',    'REPORTS — CREATE',               false),
  ('REPORTS',     'UPDATE',    'REPORTS — UPDATE',               false),
  ('REPORTS',     'DELETE',    'REPORTS — DELETE',               false),
  ('REPORTS',     'VALIDATE',  'REPORTS — VALIDATE',             false),
  ('REPORTS',     'CONFIGURE', 'REPORTS — CONFIGURE',            false),
  ('REPORTS',     'EXPORT',    'Exporter les rapports',          false),
  ('RECEIVABLES', 'READ',      'Voir les créances',              false),
  ('RECEIVABLES', 'CREATE',    'Créer des encaissements',        false),
  ('RECEIVABLES', 'UPDATE',    'Modifier créances',              false),
  ('RECEIVABLES', 'DELETE',    'Supprimer créances',             false),
  ('RECEIVABLES', 'VALIDATE',  'Valider encaissements',          false),
  ('RECEIVABLES', 'CONFIGURE', 'RECEIVABLES — CONFIGURE',        false),
  ('RECEIVABLES', 'EXPORT',    'Exporter créances',              false)
ON CONFLICT (module, action) DO NOTHING;

-- ── Seed : Role permissions (matrice) ────────────────────────────────────────
...
-- MANAGER : tout sauf ADMIN:CONFIGURE et ADMIN:DELETE
INSERT INTO rbac_role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM rbac_roles r, rbac_permissions p
WHERE r.name = 'MANAGER'
  AND NOT (p.module = 'ADMIN' AND p.action IN ('CONFIGURE', 'DELETE'))
ON CONFLICT DO NOTHING;
...
WHERE r.name = 'COMMERCIAL'
  AND (
    (p.module = 'COMMERCIAL' AND p.action IN ('READ', 'CREATE', 'UPDATE')) OR
    (p.module = 'STOCK'      AND p.action = 'READ') OR
    (p.module = 'REPORTS'    AND p.action = 'READ')
  )
ON CONFLICT DO NOTHING;
...
WHERE r.name = 'MAGASINIER'
  AND (
    (p.module = 'STOCK'     AND p.action IN ('READ', 'CREATE', 'UPDATE')) OR
    (p.module = 'LOGISTICS' AND p.action IN ('READ', 'CREATE', 'UPDATE'))
  )
ON CONFLICT DO NOTHING;
...
SELECT r.id, p.id
FROM rbac_roles r, rbac_permissions p
WHERE r.name = 'FORMATEUR'
  AND (
    (p.module = 'HR'      AND p.action IN ('READ', 'CREATE', 'UPDATE')) OR
    (p.module = 'REPORTS' AND p.action = 'READ')
  )
ON CONFLICT DO NOTHING;
...
SELECT migrate_existing_users_to_rbac();
