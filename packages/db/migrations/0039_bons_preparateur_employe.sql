-- 0039 : Bons de sortie - le preparateur est un employe RH, pas un profil utilisateur.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bons_sortie_preparateur_id_employes_fkey'
      AND conrelid = 'public.bons_sortie'::regclass
  ) THEN
    ALTER TABLE public.bons_sortie
      ADD CONSTRAINT bons_sortie_preparateur_id_employes_fkey
